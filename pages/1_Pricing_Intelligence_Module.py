...
# ─────────────────────────────────────────
# SYNTHETIC ANALYTICS GENERATOR
# ─────────────────────────────────────────
@st.cache_data
def generate_synthetic_pricing_analytics(_model, _scaler, n=200):
    """Generate synthetic pricing decisions for the analytics dashboard."""
    np.random.seed(42)
    records = []
    for _ in range(n):
        rating        = np.random.uniform(1.0, 5.0)
        count         = int(np.random.randint(10, 1001))
        ratio         = np.random.uniform(0.7, 1.3)
        current_price = np.random.uniform(20.0, 200.0)
        competitor_price = current_price / ratio

        X        = np.array([[rating, count, ratio]])
        X_scaled = _scaler.transform(X)
        action_id  = int(_model.predict(X_scaled)[0])
        ...
        records.append({
            "action":            action,
            "positioning":       ("Overpriced" if ratio > 1.1
                                  else ("Underpriced" if ratio < 0.9 else "Competitive")),
...
if model and scaler:
    st.markdown("---")
    st.subheader("📊 Pricing Analytics Dashboard")
    st.caption("Based on 200 synthetic pricing decisions generated by the XGBoost pricing model")

    df_analytics = generate_synthetic_pricing_analytics(model, scaler)
    ...
    with row1_col1:
        avg_vals = [
            df_analytics["current_price"].mean(),
            df_analytics["competitor_price"].mean(),
            df_analytics["recommended_price"].mean(),
        ]
        fig1 = px.bar(
            x=["Current", "Competitor", "Recommended"],
            y=avg_vals,
            color=["Current", "Competitor", "Recommended"],
            color_discrete_map={
                "Current": "#6b7280",
                "Competitor": "#3b82f6",
                "Recommended": "#10b981",
        ...
        fig1.update_layout(paper_bgcolor="#0e1117", font_color="white", showlegend=False)
        st.plotly_chart(fig1, use_container_width=True)

    with row1_col2:
        action_cnt = df_analytics["action"].value_counts().reset_index()
        action_cnt.columns = ["action", "count"]
        fig2 = px.pie(
            action_cnt, values="count", names="action",
    ...
    with row2_col1:
        pos_cnt = df_analytics["positioning"].value_counts().reset_index()
        pos_cnt.columns = ["positioning", "count"]
        pos_colors = {
            "Overpriced": "#ef4444",
            "Competitive": "#10b981",
            "Underpriced": "#3b82f6",
        ...
        fig3 = px.bar(
            pos_cnt, x="positioning", y="count",
            title="Market Positioning",
            color="positioning",
            color_discrete_map=pos_colors,
            text_auto=True,
        )
        fig3.update_layout(paper_bgcolor="#0e1117", font_color="white", showlegend=False)
